...
# Zipping the bundle
cd $PRESENT_WORKING_DIRECTORY/server
zip -r $PRESENT_WORKING_DIRECTORY/$ZIP_NAME content aws.js server.js package.json

# Upload to S3 for deployment to EB
aws s3 cp $PRESENT_WORKING_DIRECTORY/$ZIP_NAME $S3_DEPLOY_URL
